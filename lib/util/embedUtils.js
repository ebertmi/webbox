 ...
 */
export async function getEmbedMetadata(embedId) {
  let embed;

  // retriev embed first
  try {
    embed = await CodeEmbed.get(embedId).pluck('id', 'slug', {'meta': ['name', 'language', 'embedType']}, 'lastUpdate').execute();
  } catch (e) {

    // Try slug next
    try {
      embed = await CodeEmbed.filter({slug: embedId}).nth(0).pluck('id', 'slug', {'meta': ['name', 'language', 'embedType']}, 'lastUpdate').execute();
    } catch (ex) {
...
export async function getEmbedAndOrDocument(embedId, userId, showOriginal = false, documentId) {
  let embed;
  let document;
  let id = embedId;
  ...
  // retriev embed first
  try {
    embed = await CodeEmbed.get(embedId).getJoin({creators: true}).pluck({
    ...
    // Try slug next
    try {
      embed = await CodeEmbed.filter({slug: embedId}).getJoin({creators: true}).nth(0).pluck({
        id: true, code: true, _creatorId: true, slug: true, createdAt: true, lastUpdate: true, meta: true, assets: true,
        creators: ['id', 'email']
      }).execute();
      id = embed.id;
  ...
  if (documentId != null) {
    try {
      document = await CodeDocument.get(documentId).run();
    } catch (e) {
  ...
  try {
    document = await CodeDocument.filter({
      codeEmbedId: id,
      _ownerId: userId
    }).nth(0).run();
