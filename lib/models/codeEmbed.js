...
CodeEmbed.hasAndBelongsToMany(User, 'creators', '_creatorId', 'id');
 ...
 */
CodeEmbed.define('addCreator', function addCreator(userId) {
  return this.addRelation('creators', {id: userId}).run();
});

/**
 * Removes the user as a creator from the embed
 */
CodeEmbed.define('removeCreator', function addCreator(userId) {
  return this.removeRelation('creators', {id: userId}).run();
});

/**
 * Returns default metadata for an embed. Utility for creating new embeds.
 */
CodeEmbed.defineStatic('getDefaultMeta', function (name, langauge='python3', embedType=EmbedTypes.Sourcebox, mainFile='main.py') {
  return {
    name: name,
    language: langauge,
    embedType: embedType,
...
CodeEmbed.defineStatic('getBySlug', function (slug) {
  return CodeEmbed.filter({slug: slug}).nth(0).run();
});
 ...
 * (Helper utility)
 */
CodeEmbed.defineStatic('countBySlug', function (slug, ignore=[]) {
