...
/*
  Configures authentification based on session and admin scope
  If user is not logged in, the page should not be redirected to login (see API_PLUGINS)
*/
...
module.exports = [
  {
    ...
    method: 'GET',
    path: API_PREFIX + '/user/{id}/resetPasswordManually',
    handler: Api.resetUserPasswordManually,
    config: {
      auth: API_AUTH_CONFIG,
      validate: {
        params: {
    ...
    handler: Api.resendConfirmationEmail,
    config: {
      auth: API_AUTH_CONFIG,
      validate: {
        params: {
