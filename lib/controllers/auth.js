...
const debug = Debug('webbox:auth-controller');
...
module.exports = {
  ...
  login: async function login (request, h) {
    ...
    try {
      ...
      if (result.isValid === true) {
        ...
        const reason = (result.credentials != null && result.credentials.isVerified === false) ? Config.messages.login.notVerified : Config.messages.login.invalidCredentials;
  ...
  signup: async function signup (request, h) {
    ...
    try {
      ...
      console.log('Result from user search', email, users);
