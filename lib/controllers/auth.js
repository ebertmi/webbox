...
module.exports = {
  ...
  confirm: function (request, reply) {
    ...
    JWT.verify(token, Config.auth.verificationSecret, (err, data) => {
      if (err) {
        // invalid token!!!!
        context.errorMessage = 'Der Aktivierungsschlüssel ist ungültig.';
        return reply.view('index', context);
      }

      console.log('Registration.confirm:', 'Decoded token data', data);
      email = data.email;
      ...
      .then(users => {
        if (users.length === 0) {
          ...
          console.log('Registration.confirm: ', 'Received token with email, that does not exist.');
          context.errorMessage = 'Der Aktivierungsschlüssel ist ungültig.';
          return reply.view('index', context);
        } else {
          ...
          // update user
          user.isActive = true;
          user.verification.isCompleted = true;
          user.verification.token = undefined;
          user.roles.push('user'); // add user default role

          return user.save();
        }
      })
      .then(res => {
        context.infoMessage = 'Aktivierung erfolgreich abgeschlossen. Sie können sich nun einloggen.';
        return reply.view('index', context);
      })
      .error(err => {
        console.log('Registration.confirm: ', err);
        context.errorMessage = 'Der Aktivierungsschlüssel ist ungültig.';
        return reply.view('index', context);
      });
    });
