...
module.exports = {
        ...
        return Course.getCourseSlugsForUserId(result.credentials.id).then((slugs) => {
          ...
          request.server.app.cache.set(sid, {credentials: result.credentials}, 0, (err) => {
            if (err) {
              ...
              Log.createLog('Auth.Cache', 'Caching error while setting credentials', credentials);
  ...
  signup: function (request, reply) {
    ...
    // Steps
    console.log('Authentication.Signup', 'Starting signup process', email);
    ...
    .then(users => {
      ...
      .then(res => {
        ...
        // reply
        return reply.view('index', {
          infoMessage: Config.messages.verification.verificationSendMessage,
          user: ANONYMOUS_USER_CONTEXT.user,
        });
      })
      .error(err => {
        ...
        return reply.view('index', {
          errorMessage: Config.messages.verification.verificationErrorMessage,
          user: ANONYMOUS_USER_CONTEXT.user,
        });
      });
    })
