...
module.exports = {
  loginview: function (request, reply) {
    ...
    const redirectPath = encodeURIComponent(request.query.next || '/');
    ...
    .then((result) => {
      if (result.isValid === true) {
        ...
        request.server.app.cache.set(sid, {credentials: result.credentials}, 0, (err) => {
          ...
          console.info('Trying to redirect after login', nextQuery);
          return reply.redirect(nextQuery);
        });
        //});
      } else {
        let reason = result.credentials.isVerified === false ? Config.messages.login.notVerified : Config.messages.login.invalidCredentials;
  ...
  signupRedirect: function (request, reply) {
    // Redirect to home
    reply.redirect('/');
  },
  signup: function (request, reply) {
    ...
    .then(users => {
      ...
      .then(res => {
        ...
        const mail = VerificationTemplate.getVerificationEmailBody(confirmUrl, res.verification.token);
