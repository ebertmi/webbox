...
import { getEmbedAndOrDocument, getFileExtensionByLanguage, getTemplateContentByLanguage } from '../util/embedUtils';
...
export function* getEmbedsForUser (request, reply) {
  try {
    ...
    console.info("before filtering");
    try {
      embeds = yield CodeEmbed.filter({
        _creatorId: request.pre.user.id
      }).run();
    } catch (e) {
      console.info("filtering failed");
      console.error('embed.getEmbedsForUser', e);
      embeds = [];
    }

    console.info("before view reply");
...
export function* getCodeDocumentsForUser (request, reply) {
  ...
  try {
    codeDocuments = yield CodeDocument.filter({'_ownerId': request.pre.user.id}).getJoin({embed: true}).pluck('id', 'codeEmbedId', 'lastUpdate', {'embed': ['meta']}).execute();
  } catch (e) {
...
export function* getEmbed (request, reply) {
  ...
  if (embed.meta.embedType === 'sourcebox' && canAcessSourcebox === false) {
    // show redirect to login
    return reply.redirect(`/login?next=${request.path}`);
  }

  // Is user author of the embed?
  userData.isAuthor = embed._creatorId === userData.id;
  ...
  if (embed.meta.embedType === 'sourcebox') {
...
export function* deleteEmbed (request, reply) {
  ...
  try {
    yield CodeDocument.filter({codeEmbedId: embed.id}).delete();
  } catch (e) {
    console.error(e);
  }
