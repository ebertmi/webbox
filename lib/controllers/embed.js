...
export function*updateEmbed(request, reply) {
  ...
  creatorsWithValidEmail = rawCreators.filter(name => {
    const result = Joi.validate({ email: name }, EMAIL_SCHEMA);
  ...
  let newCreators;
  try {
    newCreators = yield Promise.map(creatorsWithValidEmail, (email) => {
...
export function*saveEmbed(request, reply) {
  ...
  if (canEditEmbed(embed, request.pre.user)) {
    ...
    try {
      // ToDo: we need to get the object and update accordingly

      let embedObj = yield CodeEmbed.get(embed.id).run();
      yield embedObj.merge({code: data.code}).save(); // save changes
    } catch (e) {
      console.log(e);
      response.error = Config.messages.embed.save.saveFailed;
      return reply(response);
