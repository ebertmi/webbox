...
import { getFileExtensionByLanguage, getTemplateContentByLanguage } from '../../common/util/languageUtils';
import { getEmbedAndOrDocument } from '../util/embedUtils';
import Config from '../../config/webbox.config';
import { MODES, EmbedTypes } from '../../common/constants/Embed';
import { canEditEmbed, canDeleteEmbed, canUpdateEmbed } from '../roles/embedChecks';
...
export function* getCodeDocumentsForUser (request, reply) {
  ...
  try {
    ...
    .filter({'_ownerId': request.pre.user.id}).getJoin({embed: true})
    .pluck('id', 'codeEmbedId', 'lastUpdate', {'embed': ['meta']})
    .execute()
    .map(function(ce) {
      // Time-Transformation
      let relDate = toRelativeDate(ce.lastUpdate);
 ...
 * @returns
 */
function createContextData (embed, userData, user) {
...
export function* getEmbedAjax (request, reply) {
  ...
  let embed;
  let canAcessSourcebox = request.auth.isAuthenticated;
  let userData;
  let response = {};
  ...
  const contextData = createContextData(embed, userData, request.pre.user);
...
export function* updateEmbed(request, reply) {
  ...
  if (data.slug != null && data.slug.length > 3) {
    ...
    try {
      let count = yield CodeEmbed.countBySlug(data.slug, [id]);
      if (count > 0) {
...
export function* runEmbed(request, reply) {
  ...
  // If any validation error has occured, reply with error
  if (request.pre.validation) {
    request.log(['validation', 'error'], {message: 'Failed validation payload', data: request.pre.validation});
...
export function* deleteEmbed (request, reply) {
  ...
  try {
    documents = yield CodeDocument.filter({codeEmbedId: embed.id});
...
export function* exportStatistics (request, reply) {
  ...
  try {
    // Check for uuid
    validationResult = Joi.validate({idOrSlug: embedId}, SLUGORID_SCHEMA);
    if (validationResult.error == null) {
