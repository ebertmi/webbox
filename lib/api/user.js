...
const debug = Debug('webbox:api-user.js');
...
export async function getUsers (request, h) {
  ...
  if (isSearch) {
    ...
    if (parsedQuery.hasFilters === true) {
      query = query.filter(createDynamicFilterFunction(parsedQuery.filters), {default: true});
    }
...
export async function resendConfirmationEmail (request, h) {
  ...
  try {
    ...
    const confirmUrl = `${request.server.info.protocol}://${request.info.host}/confirm`;
...
export async function confirmUser (request, h) {
  ...
  try {
    ...
    MailService.sendHtmlEmail('trycoding Freischaltung', template, email);
...
export async function resetUserPasswordManually(request, h) {
  ...
  try {
    ...
    let template;
    try {
      baseRoute = `${request.headers['x-forwarded-proto'] || request.server.info.protocol}://${request.info.host}`;
      template = getManualPasswordResetBody(baseRoute, newPassword);
    } catch (e) {
      debug('Failed to extract baseRoute to generate links in e-mail');
    }

    MailService.sendHtmlEmail('trycoding  - Password wurde zurückgesetzt', template, email);
