...
export async function getDocuments (request, h) {
  ...
  try {
    ...
    documents = await query.slice(sliceStart, sliceEnd).getJoin({
      creator: true
    }).run();
    if (isSearch) {
    ...
    // Add creator object in case join failed
    response.documents = documents.map(doc => {
      ...
      delete doc.creator.password;
      delete doc.creator.recentDocuments;
      delete doc.creator.verification;
      delete doc.creator.forgot;
